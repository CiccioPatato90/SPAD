...
% =========================================================================
file = 'C:\Users\Utente\Desktop\Progetto\EuCap26\Funzione di costo\Mattia\E6_2024.nc';
lon = ncread(file,'lon');
lat = ncread(file,'lat');
...
% =========================================================================
T_export = table(t_path', distanza_spad', A_z_rel', 'VariableNames', {'Time', 'Depth', 'Acceleration'});

% Percorso di salvataggio richiesto
save_dir = 'C:\Users\Utente\Desktop\Progetto\SPAD Repo\SPAD\';

% Controllo di sicurezza: se la cartella non esiste, MATLAB la crea automaticamente
if ~exist(save_dir, 'dir')
    mkdir(save_dir);
end

% Creazione del nome file dinamico. 
tau_str = strrep(num2str(tau_drone), '.', '-');
filename = sprintf('ritardo_%ss_profile.csv', tau_str);

% Unione del percorso e del nome file in modo sicuro per Windows
full_path = fullfile(save_dir, filename);

% Salvataggio
writetable(T_export, full_path);
fprintf('File generato con successo!\nNome: %s\nPercorso: %s\n', filename, save_dir);
